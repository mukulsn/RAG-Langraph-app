...
This moduel handles document relevance evaluation as part of the LangGraph RAG
pipeline. It determines whether retrieved documents contain enough relevant
information to ansewr a user's question effectively.

The evalation chain is a key compnenet in LangGraph RAG systems, providing
quality gates that prevent irrelevant documents from being used for ansewr
generation. This improves the overall quality fo RAG responses.

Used withing the LangGraph workflow to make routing deicisions about whether
...
class EvaluateDocs(BasedModel):
    """
    Document evalutaion results for LangGraph RAG workflows
...
system = """You are an expert document relevance evaluator for a RAG (Retrieval-Augmented Generation) system. Your role is to assess whether retrieved documents contains sufficient information to answer a user's query effectively.
...
3. INFORMATION QUALITY:
    ...
    - Are there conflicting statements withing the documents?
...
Be thorough but efficient in your evaluation. Focus on practical utility for ansewr generation."""

evaluate_prompt = ChatPromptTemplate.from_message(
    [
        ("system", system),
        ("human", """Please evaluate whether the retrieved documents are sufficient to answer the user's query.
         ...
         4. Missing Information: What key information (if any) is misisng for a complete answer?
