...
class StreamlitMultiFormatDocumentLoader:
    ...
    def __inti__(self):
        """Initialise with the base multi-format loader"""
    ...
    def load_uploaded_file(self, uploaded_file) -> List[Document]:
        ...
        try:
            logger.info(f"Processing uploaded file: {uploaded_file.name} (size: {len(uploaded_file.get_value())} bytes)")
    ...
    def load_multiple_uploaded_files(self, uploaded_files) -> List[Document]:
        ...
        Args:
            uploaded_files: List of Streamlit uploaded file objets
        ...
        logger.info(f"Total: {len(all_documents)} document chunks from {len(uploaded_files) - len(failed_files)} successfull uploads")
